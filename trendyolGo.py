...
class ToggleSwitch(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Arka plan
        if self._checked:
            painter.setBrush(QColor("#f27a1a"))  # Açıkken TrendyolGo turuncusu
        else:
...
class TrendyolGoPage(QWidget):
    def __init__(self, parent=None):
        ...
        # Geri ikonu (sabit turuncu renk)
        back_icon = QLabel()
        default_pixmap = QIcon("svg/back.svg").pixmap(20, 20)
        turuncu_pixmap = QPixmap(20, 20)
        turuncu_pixmap.fill(Qt.transparent)
        painter = QPainter(turuncu_pixmap)
        painter.drawPixmap(0, 0, default_pixmap)
        painter.setCompositionMode(QPainter.CompositionMode_SourceIn)
        painter.fillRect(turuncu_pixmap.rect(), QColor("#f27a1a"))
        painter.end()
        back_icon.setPixmap(turuncu_pixmap)
        ...
        pixmap = QPixmap("trendyol.png").scaled(30, 30, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        logo_label.setPixmap(pixmap)
        logo_label.setStyleSheet("background-color: transparent;")

        trendyolgo_label = QLabel("TrendyolGo")
        trendyolgo_label.setStyleSheet("font-size: 18px; font-weight: bold; color: #333;")

        logo_text_layout = QHBoxLayout()
        logo_text_layout.setSpacing(5)
        logo_text_layout.addWidget(logo_label)
        logo_text_layout.addWidget(trendyolgo_label)
        ...
        customer_icon.setText("R")  # İlk harf (örneğin "Kayra D." için "K")
        ...
        start_button.setStyleSheet("""
            QPushButton {
                background-color: #f27a1a;  /* TrendyolGo turuncusu */
                border: none;
                border-radius: 5px;
                padding: 8px;
                font-size: 14px;
            ...
            QPushButton:hover {
                background-color: #d66a17;  /* Hover'da daha koyu turuncu */
            }
        """)
